...
def get_links(text):
    #для использования поддельного браузера
    ua =fake_useragent.UserAgent()
    res = requests.get(
        url=f"https://hh.ru/search/resume?text={text}&area=54&isDefaultArea=true&exp_period=all_time&logic=normal&pos=full_text&fromSearchLine=false&page=1",
    ...
    try:
        ...
        return
    for page in range(page_count): #цикл по всем страницам
        try:
            res = requests.get(
...
if __name__ == "__main__":
    ...
    for a in get_links("python"):
        ...
        with open("data.json","w",encoding="utf-8")as f:
            json.dump(data,f,indent = 4, ensure_ascii=False)#ensure_ascii-что бы русские буквы не заменялись кодами